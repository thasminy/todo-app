...
export default function EditTodoPage() {
  ...
  const item = todos.find((t: Todo) => t.id === id);
  ...
  const [category, setCategory] = useState(item?.category || "Work");
  const [showModal, setShowModal] = useState(false);

  if (!item) return <div>Todo not found</div>;

  const handleSave = () => {
    editTodo(item.id, { title, startDate, startTime, endDate, endTime, category });
    setShowModal(true);
    setTimeout(() => {
      setShowModal(false);
      router.push("/todos");
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            <h3 style={{ marginBottom: "8px", color: "#1976d2" }}>Task Updated!</h3>
            <p style={{ color: "#232526" }}>Your changes were successfully saved.</p>
          </div>
        </div>
      )}
    </div>
